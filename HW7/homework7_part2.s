...
_start:      
        call  CONFIG_STACK /* Initialize stack pointer */
        call  INITIALIZE_COUNTER /* Initialize millisecond counter */
        call  CONFIG_JTAG /* Initialize JTAG_UART */
        call  CONFIG_TIMER /* Initialize TIMER */
        call  CONFIG_INTERRUPT /* Initialize interrupts */
...
INITIALIZE_COUNTER: /* These registers remain unchanged throughout program */
        ...
        movi  r16,0x3E8 /* terminating milliseconds count value */
        movi  r17,0x63 /* Code for C (Clear) */
        movi  r18,0x73 /* Code for S (Start) */
        movi  r19,0x70 /* Code for P (Stop) */
        ret /* return from subroutine */
...
CONFIG_JTAG:
        movia  r3,JTAG_UART /* Load JTAG_UART address */
        movi   r2,0x401 /* Clear AC Bit, Enable Read interrupts */
        stwio  r2,JTAG_CONTROL(r3) /*  */
        ret /* return from subroutine */

CONFIG_TIMER:
        /* Stop Timer */
        movia  r3,TIMER /* Load TIMER address */
        movi   r2,0b1000 /* Stop bit */
        stwio  r2,TIMER_CONTROL(r3) /* Stop TIMER */
        movi   r2,0b0001 /* TO bit */
        stwio  r2,TIMER_STATUS(r3) /* Clear TO bit */
        movia  r3,TIMER_2sDELAY /* Load 2 sec delay address */
        ldwio  r2,(r3) /* Load 2 sec delay */
        sthio  r2,TIMER_PERIODL(r3) /* low 2 bytes to PERIODL */
        srli   r2,r2,16 /* Logical shift right 2 bytes */
        sthio  r2,TIMER_PERIODH(r3) /* high 2 bytes to PERIODH */
        ret /* return from subroutine */

CONFIG_INTERRUPT:
        /* ctl4 */
        movi  r2,0b1000 /* IRQ 0 for TIMER, IRQ 1 for KEYS */
